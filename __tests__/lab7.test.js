describe('Basic user flow for Website', () => {
  ...
  it('Make sure <product-item> elements are populated', async () => {
    ...
    for(let i = 0; i < prodItemsData.length; i++) {
        ...
        let currentItem = prodItemsData[i];
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    const shadowRoot = await product.getProperties('shadowRoot');
    const button = await product.evaluateHandle(el => el.shadowRoot.querySelector('button'));
    await button.click();
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    for(let i = 0; i < products.length; i++) {
      const shadowRoot = await products[i].getProperty('shadowRoot');
      const button = await products[i].evaluateHandle(el => el.shadowRoot.querySelector('button'));
      const buttonText = await button.getProperty('innerText');
      const buttonTextValue = await buttonText.jsonValue();
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    const cartCount = await page.$('#cart-count');
    expect(await cartCount.evaluate(el => el.innerText)).toBe('20');
  ...
  it('Checking number of items in cart on screen after removing from cart', async () => {
     ...
     * Remember to remove the .sst cart = await page.evaluate(() => {kip from this it once you are finished writing this test.
     */
    await page.evaluate(() => {
    ...
    const cartCount = await page.$('#cart-count');
    const count = await cartCount.evaluate(el => el.innerText);
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    const cartCount = await page.$('#cart-count');
    expect(await cartCount.evaluate(el => el.innerText)).toBe('0');
